...
| v0.8 | 2026-06-11 | 디자인 감각 보강 패스 (구현 코드 기준선 전환 — 시안 HTML은 v0.7.3에서 동결, 이후 기준은 `src/` + `docs/UI_GUIDE.md`) — 사용자 만족도 ~70% 진단 5건(랜딩 약함·섹션 한 화면 초과·페이지 간 타이포/여백 불일치·studio 입력창 부유·Saved Looks 불일치) 반영. ① **`--accent` `#3d4f6b` muted slate → `#49476e` Ink Violet** (glow `#7672ac` = `--accent-glow-rgb` 신규, 사용자 옵션 4개 중 B 선택 — 에디토리얼·하이패션 무드). ② **히어로 마네킹 실사 로테이션** (ADR-018): 얼굴 없는 3D 마네킹 전신 룩 7장(WebP 840px)을 좌측 `clamp(320px,42vw,580px)` 레이어로, 2.4s 간격 rotateY(16deg) crossfade "도는 느낌", 좌(0.30)→우(0.94) 다크 스크림 + accent 글로우 + grain 합성. 글래스 룩 패널 5개 제거(실사가 대체). reduced-motion 첫 장 고정. ③ **한 화면 섹션 리듬**: how-it-works·curated-preview `min-h-[100svh]` flex 센터링 (전체 scroll-snap은 도입 안 함). ④ **Curated 가로 룩북 레일**: 3×2 그리드 → 한 줄 snap 가로 스크롤(카드 `clamp(232px,21vw,300px)`, 사용자 선택). ⑤ **SectionHeader 단일 헤더 패턴** + `px-6 md:px-gutter`(34px) 가로 거터 표준 + HowItWorks ad-hoc px(58/30) → t2/t3 토큰화 ⑥ **Studio 프롬프트 도크 통합("질문이 곧 무대")**: 풀블리드 sticky 스트립 폐기 → 헤딩 직하 `max-w-[760px]` 블록, sticky는 도크 pill만(셸 = 공용 `.glass-search`), `size=lg`(input h-14·t4·버튼 56px), 시즌 칩 non-sticky — 입력창 부유감 제거 ⑦ **Saved Looks 통일**: 헤더 SectionHeader compact(eyebrow t8→t7, h1 t3), 그리드 gap 3.5·거터 표준 — curated와 동일 어휘 |
