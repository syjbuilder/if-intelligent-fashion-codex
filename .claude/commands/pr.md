...
2. **사전 체크**:
   ...
   - **현재 브랜치가 main이면**: "main에서 PR을 만들 수 없습니다. 어떤 브랜치명으로 분기할까요? (예: `feat/디자인-v0.5`, `docs/sync`, `fix/홈페이지-여백`)" 질문 → 답 받으면 `git checkout -b <branch>` 실행. 이후 커밋이 main에만 있고 새 브랜치엔 없을 수 있는데, 새 브랜치 = 현재 main HEAD에서 분기되므로 OK
