...
**결정**: V0의 핵심 데이터 단위는 개별 상품이 아니라 전체 룩. curated look DB 500개로 시작. (단, V0 *첫 증명*은 룩 ~50 규모로 시작하고 500은 증명 후 확장 — ADR-020에서 V0 분량 재조정. 룩 단위·수동 큐레이션 원칙 자체는 유지.)
...
### ADR-018: 정적 마네킹 룩 실사 이미지 도입 — 브랜드 표면(히어로)에 한정

**결정**: 랜딩 히어로에 **얼굴 없는 3D 마네킹 전신 룩 실사 이미지**(사전 생성·정적 자산)를 도입한다. `scripts/build-look-images.mjs`로 840px WebP(q80)로 변환해 `public/looks/`에 커밋하고, `next/image`로 서빙한다. 적용 범위는 브랜드 표면(히어로 배경 로테이션)에 한정하며, 룩 카드(tone-* 그라디언트 + 가먼트 SVG)는 유지한다.

**이유**:
- **(1)** 패션 서비스의 첫 화면이 추상 SVG만으로는 "감각적·여운" 기준(UI_GUIDE 원칙 1)을 못 채운다는 사용자 평가(만족도 ~70%). 실사 룩이 "Wear what you imagine"을 즉물적으로 보여준다.
- **(2)** 얼굴 없는 마네킹은 ADR-006의 금지 사유(사람 사진·인종/지역 마커·초상권)에 걸리지 않는다 — ADR-006의 예외가 아니라 경계 안의 선택.
- **(3)** 정적 자산(총 ~180KB WebP 7장)이라 런타임 비용·외부 의존 0. 빌드 파이프라인은 1회성 스크립트.

**트레이드오프**: 정석은 CDN/이미지 서버 분리지만, 장당 20-40KB·7장 수준에서는 git 커밋 + Vercel 정적 서빙이 단순하고 충분하다. 원본 PNG(장당 ~1.8MB)는 레포에 넣지 않는다.

**재검토 트리거**: 정적 룩 이미지가 수십 장 이상으로 늘거나(Explore 카드 실사화 등) AI 생성 룩 이미지(Supabase Storage)와 합류할 때 — 그땐 Storage/CDN 일원화를 검토한다.
...
**후속**: 가입 grant는 트리거가 `users.token_balance=10`만 기록하고 `token_transactions` grant 행은 미기록(DATA_MODEL §15.8 갭). 동작 무관이라 보류 — generation 토큰 차감(ADR-014) 배선 시 후속 마이그레이션으로 함께 추가한다. (마이그레이션 008은 `users.email` nullable 완화에 사용 — Kakao 등 이메일 미보유 대비 방어적 조치. Kakao는 개인 비즈앱 전환으로 `account_email` 수집.)
...
### ADR-020: V0 첫 마일스톤을 "500 적재"에서 "핵심 루프 소량 end-to-end 증명"으로 전환

**결정**:
- **(1) V0 첫 마일스톤 재정의**: "curated 500개 적재"가 아니라 **핵심 루프(자유 프롬프트 → 룩 → 실제 상품)를 소량으로 처음부터 끝까지 증명**하는 것을 V0의 첫 목표로 한다. 증명 규모 = **룩 ~50 + 실상품 카탈로그 ~50 + 자유 프롬프트 평가셋 ~25**. 500은 *증명 후 확장*으로 재배치(ADR-005의 룩 단위·수동 큐레이션 원칙은 유지, V0 분량만 축소).
- **(2) Human 평가셋을 V0로 승격**: 자유 프롬프트 ~25개에 "맞음/안맞음" 라벨을 매기는 평가 루프를 V0.5에서 **V0로** 끌어올린다. 매칭 임계값(0.70)·차원 가중치를 손볼 정량 근거이자 "B-path 검색만으로 충분한가, 실시간 생성이 진짜 핵심인가"를 숫자로 판정하는 도구이기 때문.
- **(3) A-path 상품 "텍스트 검색"을 V0로**: 생성 이미지 기반 상품 검색을 **텍스트(태그·캡션) 검색판 = V0** / **이미지 임베딩 정밀판 = V0.5**로 분리하고, 텍스트판을 V0에 포함한다. AI가 가장 필요한 순간(B-path 미스)에 상품 연결이 비는 구멍을 V0에서 메우기 위함.
- **(4) 매칭기를 V0 1급 컴포넌트로**: 규칙 기반 프롬프트 의도 해석 + 룩 매칭기(6차원 가중합, ≥0.70)를 V0의 명시적 1급 컴포넌트로 두고, (2)의 평가셋으로 튜닝한다(`docs/AI_PIPELINE.md` §4).
- **(5) 빌드 순서 고정**: ① 키 0원 매칭 슬라이스(규칙 의도해석 + 매칭기 + 실상품 패널) → ② 평가셋 측정 → ③ 결과 보고 A-path(OpenAI 키) 점등. **0·1·2단계는 키 0원, OpenAI 키는 3단계에만.**

**이유**: 기존 계획은 "데이터 500 배관"을 먼저 깔았으나 핵심 차별점인 ②프롬프트→③룩→④상품 루프를 한 번도 end-to-end로 증명하지 못했다. 핵심 모순 — 목표는 "자유 프롬프트"인데 B-path(미리 만든 룩 검색)는 자유 입력을 정해진 단어로 좁혀 흔한 말엔 맞지만 독특할수록 빗나가고, 빗나간 말일수록 A-path로 가는데 거기서 상품 연결이 가장 약하다(문서가 V0.5로 미뤄둠). 즉 **AI가 진짜 필요한 순간일수록 상품 연결이 약해지는 구조**다. 이를 V0에서 메우고, "충분한가"를 추측이 아니라 평가셋 숫자로 판정해야 500·키 투자가 정당화된다.

**트레이드오프**: ADR-005(룩 단위·수동 큐레이션)의 정신은 유지하되 V0 *분량만* ~50으로 축소 → 초기 다양성은 낮으나 증명 후 확장. 1단계 룩 이미지는 placeholder(기존 마네킹 webp 재사용, ADR-006/018) 임시 사용 → 미려한 실시간 생성은 3단계 A-path(ADR-003)로 미룬다. 평가셋 라벨링은 사람(PO) 손이 들지만, 그 비용이 "튜닝 근거 없는 가중치 추측"보다 싸다. 확장 배관(시드 파이프라인·임베딩·DB 적재)을 증명 전에 깔지 않아 매몰비용 위험을 회피한다(슬라이스 데이터는 in-repo 시드로 시작, 증명 후 Supabase 적재).

**재검토 트리거**: Stage 2 평가셋 적중률 측정 후 — ① 500개 확장 착수 여부 ② OpenAI 키 발급·A-path 점등 여부 ③ 0.70 임계값·6차원 가중치 재조정 ④ 이미지 임베딩 매칭(V0.5) 승격 여부를 데이터로 결정한다.

**관련**: ADR-003(A-path 모델 고정), ADR-005(룩 단위 — 정신 유지·V0 분량 축소), ADR-008(상품 수동 큐레이션 SOP), ADR-017(이미지 buy), `docs/AI_PIPELINE.md` §4(매칭 가중합·0.70)·§5(Human eval)·§8(개발 우선순위)·§9(수집 SOP), `docs/PRD.md`(핵심기능 #4·V0 Acceptance).
