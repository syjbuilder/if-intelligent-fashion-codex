...
- **런타임**: Node.js runtime 라우트 핸들러. `maxDuration`을 60초 목표로 상향(단, Vercel 플랜 한도 내 — 열린 항목, 아래 환경/마이그레이션 절차 참조).
...
1. **OpenAI billing alert** — OpenAI 대시보드의 billing hard limit + email alert. 무료·PO 수작업, Phase 5 진입 전 설정.
